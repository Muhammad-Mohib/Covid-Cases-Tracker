...
export default function Home() {
  ...
  const GetData = () => {
    ...
    if (country.trim().length !== 0) {
      ...
      axios
        .get("https://covid19.mathdro.id/api/countries/" + UserCountry, {
          crossdomain: true,
        })
        .then(function (response) {
          setCases(response.data.confirmed.value);
  ...
  return (
    <div>
      <h1 className={"tc tw mt"} style={{ textTransform: "uppercase" }}>
        {UserCountry}
      </h1>
      ...
      <div className={"center"}>
        <div className="flex-container">
          <div>
            <h1>Total Cases: {FormatCases(totalCases)}</h1>
          </div>
          <div>
            <h1>Recovered: {FormatCases(totalRecovered)}</h1>
          </div>
          <div>
            <h1>Deaths: {FormatCases(totalDeaths)}</h1>
          </div>
        </div>
      </div>
